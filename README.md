...
The diagram below shows the flow of the Jenkins jobs.You run the `test.yml` playbook, this will run some tests. If the instance passes the tests, then it merges with main. If it fails the test then Jenkins will run the `test_solution.yml` playbook, this will fix the instance. After it is merged with main the playbook `svc.yml` will be run. This will return written output of the services status, which will be pulled by Grafana and used in logs.
